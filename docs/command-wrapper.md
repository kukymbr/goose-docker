...
1. creating your own Dockerfile:
   ```Dockerfile
   FROM ghcr.io/kukymbr/goose-docker-cmd:v3.27.0
...
2. using a volume:
   ```shell
   docker run --rm ghcr.io/kukymbr/goose-docker-cmd:v3.27.0 \
...
docker run --rm -v ./path/to/your/migrations:/migrations \
     ...
     ghcr.io/kukymbr/goose-docker-cmd:v3.27.0 up
```
...
```shell
docker run --rm -v ./path/to/your/migrations:/migrations \
     ghcr.io/kukymbr/goose-docker-cmd:v3.27.0 \
...
services:
  ...
  migrations:
    image: ghcr.io/kukymbr/goose-docker-cmd:v3.27.0
    environment:
